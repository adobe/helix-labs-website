...
function createTable(results, averages) {
  ...
  keys.forEach((key) => {
    ...
    const deviation = stddev(keytoarray(results, key));
    if (key === 'CLS') {
      value = Math.round(value * 1000) / 1000;
  ...
  const scoreCircleHtml = `<div class="score-container"><div class="score-circle" style="background-color: ${avgScoreInfo.color}; color: white;">${avgScore}</div></div>`;
